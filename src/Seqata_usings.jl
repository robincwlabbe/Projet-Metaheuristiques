...
if USING_ALL
    ...
    _xsv = Args.get("external_mip_solver") # e.g.  :cplex, :glpk, ...
    _need_jump = false
    ...
    if MS_LOG && ln1()
        ...
        println("   external_mip_solver: _xsv=$(_xsv))")
    ...
    if _need_jump
        ...
        # Le solveur externe à charger dépend de l'option :external_mip_solver
        if _xsv == :glpk
            @ms using GLPK
        elseif _xsv == :cplex
            @ms using CPLEX
        ...
        elseif _xsv == :tulip
            ...
            error("external_mip_solver inconnu : $_xsv")
        end
    end
