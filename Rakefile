...
# - xx/xx/2014: création pour coorigé projet pour Alain Faye
#
...
# if appdir.match?(/\/work\/Seqata.jl$/)
#   DESTDOC = "#{TOPDIR}/www_minisme/docs/seqata_docs/"
# else
#   DESTDOC = "#{TOPDIR}/www_minisme/docs/proto_docs/"
# end
# #
...
# desc "Envoie les docs vers sod324.minisme.fr"
# NON: C'EST À FAIRE MANUELLEMENT !
# task :push_to_minisme do
#   do_push_to_minisme
# end
...
def do_docs_make()
  ...
  # On veut récupérer la chaine "seqata" à partir du nom du répertoire `Seqata.jl`
...
# 
def do_proto(branch:"proto_p2022", arc_bname:"proto_seqata_p2022")
  if ENV['USER'] != "diam"
    puts "\nDésolé, cette action n'est utilisable que par l'utilisateur \"diam\""
    exit
  end
  ...
  # Conclusion : éviter les métacaractère quand c'est possible
  pats_to_delete = %w(
    ADMIN
    .git
  )
  ...
  cmd = %W(rm -r #{arc_dated_name}) # pas de métacaractère dans arc_dated_name
  puts "TODO: "
  ap cmd
  sh *cmd
